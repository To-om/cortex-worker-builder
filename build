...
def build_docker(args, analyzer_name, flavor):
    def build(dockerfile):
        args.docker_client.images.build(
...
def docker_push_image(args, repo, tag):
    ...
    print('Push Docker image {}:{}'.format(image, image_tag))
...
def main():
    ...
    parser.add_argument('-n', '--namespace', required=namespace is None, default=namespace,    help='Namespace of docker images')
    parser.add_argument('-u', '--user',      required=user is None,      default=user,         help='Username to authenticate to the Docker registry')
    parser.add_argument('-p', '--password',  required=password is None,  default=password,     help='Password to authenticate to the Docker registry')
